...
const CellComponent: React.FC<CellComponentProps> = ({
    ...
    const displayPlayers = playersOnCell.length > 1
        ? playersOnCell.filter((_, index) => players.findIndex(p => p.id === playersOnCell[index].id) === currentPlayer)
        : playersOnCell;
