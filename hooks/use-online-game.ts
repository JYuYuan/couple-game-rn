...
export const useOnlineGame = (config: OnlineGameConfig) => {
  ...
  }, [socket, onPlayerMoveReceived, onTaskTriggerReceived, onTaskCompleteReceived, onGameStartReceived]);
  ...
  const rollDiceOnline = useCallback((diceValue: number) => {
  ...
  const movePlayerOnline = useCallback((playerId: string, fromPosition: number, toPosition: number, steps: number) => {
    if (room) {
  ...
  const completeTaskOnline = useCallback((taskId: string, playerId: string, completed: boolean, rewardSteps?: number) => {
    if (room) {
  ...
  // 获取当前玩家
  const getCurrentPlayer = useCallback((): OnlinePlayer | null => {
    if (!room || !socket.currentPlayer) return null;
    return room.players.find(p => p.socketId === socket.currentPlayer?.socketId) || null;
  }, [room, socket.currentPlayer]);
  ...
  const isCurrentPlayerTurn = useCallback((): boolean => {
    ...
    return currentTurnPlayer?.socketId === socket.currentPlayer.socketId;
  }, [room, socket.currentPlayer]);
