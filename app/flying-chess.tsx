...
export default function FlyingChessGame() {
  ...
  const checkCellAndTriggerTask = (playerId: number, position: number) => {
    ...
    // 检查是否有其他玩家在相同位置（碰撞）
    const playersAtPosition = players.filter(
      (p) => p && p.position === position && p.id !== playerId,
    )
  ...
  const movePlayerStepByStep = (
    ...
    console.log(
      `Player ${startPlayer.id} from position ${startPosition} rolled ${steps} steps, target position: ${targetPosition}`,
    )
    ...
    return (
      <View style={[styles.container, { backgroundColor: colors.homeBackground }]}>
                ...
                <View
                  ...
                  style={[
                    styles.playerCard,
                    {
                      ...
                      borderColor:
                        gameData.currentPlayerIndex === index ? player.color : 'transparent',
        ...
        <TaskModal
          ...
          opponentPlayer={
            gameData.currentPlayer ? getOpponentPlayer(gameData.currentPlayer.id) : null
          }
      ...
      <View style={[styles.container, { backgroundColor: colors.homeBackground }]}>
        ...
        >
          ...
          <View style={[styles.statusBar, { backgroundColor: colors.homeCardBackground }]}>
            <View style={styles.statusLeft}>
              <Text style={[styles.statusTitle, { color: colors.homeCardTitle }]}>
                {onlineGameHook.gameStatus === 'waiting'
                  ? t('flyingChess.gameStatus.waiting', '准备开始')
                ...
                <View style={styles.diceWrapper}>
                  <TouchableOpacity
                    style={[
                      ...
                      {
                        backgroundColor:
                          isRolling || isMoving || !onlineGameHook.isCurrentPlayerTurn
                            ? '#FF6B6B'
                            : colors.settingsAccent,
                        borderWidth: 3,
                        borderColor: 'white',
                        opacity:
                          isRolling || isMoving || !onlineGameHook.isCurrentPlayerTurn ? 0.6 : 1,
                      },
                    ]}
                ...
                <View
                  ...
                  style={[
                    styles.playerCard,
                    {
                      backgroundColor: player.color + '15',
                      borderColor:
                        onlineGameHook.currentPlayerIndex === index ? player.color : 'transparent',
  ...
  function handleOnlinePlayerMove(steps: number) {
    ...
    // 发送移动数据到服务器
    onlineGameHook.movePlayerOnline(currentPlayer.id, startPosition, targetPosition, steps)
