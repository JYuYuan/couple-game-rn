...
class GameSimulator {
    ...
    async startCreateMode() {
        ...
        if (this.bots.length > 1) {
            ...
            const joinPromises = this.bots.slice(1).map(bot =>
                this.delayedJoin(bot, this.currentRoomId, Math.random() * 2000)
            );
    ...
    async startJoinMode() {
        ...
        const joinPromises = this.bots.map((bot, index) =>
            this.delayedJoin(bot, this.targetRoomId, index * 1000) // 每个机器人间隔1秒加入
        );
